...
- **F3.20 (fatto)** — Classificazione della schermata in un solo posto. `vision.battle_detector` diventa `vision.screen_mode` e espone una sola funzione, `classify_screen`, che ritorna `BATTLE`, `PARTY_MENU` o `OTHER` più il motivo per il tooltip; le due funzioni booleane di F3.15 e F4.6 sono diventate i suoi segnali interni. Novità vera: una sentinella testuale sul pulsante `ESCI` in basso a destra dell'elenco Pokemon, con ROI misurata per colore su due catture live di dimensioni diverse (coincidono a meno di 0.001 in coordinate normalizzate). L'OCR ci legge `ESCI` a 0.98 sul menu e spazzatura dal box messaggi sui frame di battaglia. Il guard del recognize squadra (F3.12) era a posteriori — sei slot letti, poi il verdetto — quindi pagava dodici chiamate OCR per scoprire di essere sulla schermata sbagliata, e non distingueva l'elenco da un'altra schermata a fondo teal; ora quello resta come seconda linea, perché guarda cosa è stato davvero letto e non un rettangolo. La sentinella costa un'OCR, quindi è opzionale: il poll di F4 passa `ocr=None` e si ferma al colore, dove sbagliare costa solo un giro di "non lo so".
- **F3.21 (fatto)** — Via il canale pHash: il riconoscimento è solo-nome. In combattimento la cattura ha campo e cielo dietro il Pokemon mentre le reference indicizzate stanno su bianco, quindi la specie giusta stava a distanza 20-24 e specie sbagliate a 14-16. Lato avversario la soglia 12 non veniva mai raggiunta — costo puro. Lato giocatore era dannoso: col match ristretto ai sei di squadra, a OCR muta `_combine` cadeva sul ramo solo-sprite e ritornava la distanza minima con confidenza 0.70, sopra la soglia di applicazione, evidenziando con sicurezza lo slot sbagliato. Stesso verdetto per il fallback icona del menu (F3.9): confidenza `1 - distanza/18` contro distanze 16-24, cioè ≤ 0.11 contro una soglia di 0.60 — non poteva cambiare la squadra in nessun caso. Rimossi `_combine` (sostituito da `_best_match`, che traduce lo score del nome in confidenza), le ROI `opponent_sprite`, `player_sprite` e `slot_icons`, e gli script di debug delle icone. I rettangoli da calibrare per gioco passano da 24 a 16. `find_pokemon_by_sprite_hash` resta nel repository, senza chiamanti: è la query che servirebbe se si segmentasse il soggetto dallo sfondo (§7).
- **F3.22 (fatto)** — Calibratore ROI visuale, il punto aperto più vecchio di §7. `ui/roi_calibrator.py` mostra il frame dell'emulatore e i 16 rettangoli da cui l'app legge; si disegnano col mouse con snap alla griglia nativa 240x160, si aggiustano con le frecce, e sotto compaiono il ritaglio ingrandito e cosa ci si legge dentro — OCR per il testo, `read_level` per i livelli, pixel colore-PS per la barra. Quest'ultima parte è quella che conta: entrambi i bug di calibrazione del repo erano invisibili a occhio e si smascherano alla prima lettura (spostando `opponent_name` dei 20 px di F3.18 la riga diventa `nessun testo letto`). Le ROI calibrate vivono in `%APPDATA%/pokemon-helper/rois/<gioco>.json` e vincono sui default: `vision/roi_store.py` le risolve, e tutti i chiamanti passano da lì invece di leggere `GAME_ROIS`. Lettura tutto-o-niente, perché una calibrazione applicata a metà non sarebbe né quella dell'utente né quella del repo. `vision/roi_targets.py` tiene l'elenco dei rettangoli e li indirizza per chiave dentro la struttura annidata, e `format_rois_snippet` esporta il literal Python per promuovere una calibrazione a default del repo.
...
### F5 — Abilità che modificano l'efficacia (fatto, fuori dal piano originale)

Era il punto aperto più grave per la correttezza: il calcolo guardava solo i tipi, quindi contro un Gengar con Levitazione il pannello consigliava Terra e mandava il giocatore contro un'immunità. Unico caso in cui la risposta era *errata* e non semplicemente incompleta.

- **F5.1 (fatto)** — Tabelle `abilities` e `pokemon_abilities` nel dataset, con `generation_introduced` per abilità. `PokemonRepository.get_abilities(pokemon_id, generation)` applica tre filtri: nessuna abilità prima della Gen 3, nessuna introdotta dopo la generazione richiesta, nessuna nascosta prima della Gen 5.
- **F5.2 (fatto)** — `engine/abilities.py` applica l'effetto al profilo difensivo. Copre solo le abilità che cambiano un moltiplicatore di tipo: immunità (Levitazione, i quattro "assorbi"), moltiplicatori fissi (Grassospesso, Pellearsa, Antifuoco) e modificatori sulle superefficaci (Filtro, Solidroccia, Meraviglia). `since_generation` distingue *quando l'abilità ha assunto l'effetto attuale* da *quando è comparsa*: Parafulmine e Acquascolo fino alla Gen 4 redirezionavano l'attacco senza dare immunità.
- **F5.3 (fatto)** — `ABILITY_HISTORY_OVERRIDES` in `build_dataset.py`, gemello di `TYPE_HISTORY_OVERRIDES`. veekun pubblica solo l'assegnazione corrente: Gengar ha perso Levitazione in Gen 7, quindi il CSV la ometterebbe proprio nel caso che conta. Una sola voce; le altre 30 specie con Levitazione l'hanno mantenuta.
- **F5.4 (fatto)** — UI: candidato unico applicato in automatico, più candidati risolti da un menu a tendina la cui scelta è persistita per specie in `state.json`. Il menu è evidenziato **solo se** l'ambiguità può cambiare il verdetto: Magnemite ha due abilità e nessuna tocca l'efficacia, Lanturn invece sì. Tooltip con la descrizione dell'abilità in italiano, presa dal flavor text del gioco.

Il filtro per generazione risolve da solo circa metà dei casi rilevanti in Gen 3 (28 su 58); in Gen 5 meno, perché le abilità nascoste aggiungono un terzo candidato.
...
- **Match icona menu**: rimosso in F3.21. pHash/dhash non raggiungevano distanze basse nemmeno col color-key HSV, e con la soglia di confidenza introdotta in `755cc5e` il canale non poteva più scrivere niente: era inerte, non inaffidabile.
- **pHash sprite in combattimento**: rimosso in F3.21. La cattura ha campo e cielo dietro il Pokemon mentre le reference indicizzate stanno su bianco, quindi la specie corretta non entrava nei primi tre a nessun offset di ROI. Il verdetto regge interamente sul nome. Per recuperarlo servirebbe segmentare il soggetto dallo sfondo prima del hash — i fondali di battaglia sono a bande di colore piatte, quindi un flood-fill dai bordi è plausibile — e rifare le due ROI sprite, tolte insieme al canale.
- **Abilità storiche**: `pokemon_abilities.csv` descrive l'assegnazione corrente e veekun non pubblica quella storica. `ABILITY_HISTORY_OVERRIDES` copre il caso noto (Gengar); se ne emergono altri vanno aggiunti lì, perché dai CSV non sono ricostruibili.
- **Calibrazione ROI visuale**: risolto in F3.22. Le ROI del repo restano quelle di FRLG a scala mGBA, ma non sono più l'unica sorgente: il calibratore le ridisegna sulla finestra dell'utente e le salva in `%APPDATA%`, e `vision/roi_store.py` le fa vincere sui default. Resta `scripts/extract_roi_from_annotated.py` come via offline da PNG annotato.
- **Chrome mGBA**: risolto, `vision/chrome.py` misura la fascia sul frame catturato (titolo e menu bar sono righe grigie e chiare, le schermate di gioco sono sature) e i 52 px dichiarati nel layout restano il fallback di quando la misura non convince — tema scuro, schermo bianco in transizione. Verificato sul vivo: misura 52 px, identici al valore preso a mano. Nota: è l'unica costante in pixel assoluti del sistema ROI — tutto il resto è normalizzato e quindi indipendente dalla dimensione della finestra. Quando è cambiata da 30 a 52 ha sfasato di ~20 px tutte le ROI battaglia, calibrate prima del fix; sono state riproiettate sulla game area vera (commit `1f85b1e`).
