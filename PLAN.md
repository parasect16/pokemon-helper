...
- **Match icona menu**: pHash/dhash non raggiungono distanze basse anche con color-key HSV. Da quando l'OCR gira senza il modello di detection il canale nome risolve tutti e sei gli slot, quindi il fallback icona non viene quasi mai raggiunto; la soglia di confidenza introdotta in `755cc5e` gli impedisce comunque di scrivere una specie sbagliata.
- **pHash sprite in combattimento**: inservibile. La cattura ha campo e cielo dietro il Pokemon mentre le reference indicizzate stanno su bianco, quindi la specie corretta non entra nei primi tre a nessun offset di ROI. Il verdetto regge interamente sul nome. Per recuperarlo servirebbe segmentare il soggetto dallo sfondo prima del hash: i fondali di battaglia sono a bande di colore piatte, quindi un flood-fill dai bordi è plausibile.
